            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Verify 161 authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Verify 161 failed to authenticate properly"
                throw err
            }
        }
    }
        ...
        try {
            dir(projectDirectory) {
                ...
                slackSend color: "good",
                        message: "Verify 161 deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Verify 161 failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Verify 161 shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Verify 161 failed to shut down properly"
            throw err
        }
    }
}
